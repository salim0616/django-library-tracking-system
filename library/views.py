...
class LoanViewSet(viewsets.ModelViewSet):
    ...
    def extend_due_date(self, request, pk=None):
        ...
        try:
            additional_days_needed = int(request.data.get("additional_days", None))
        except ValueError:
            return Response(
                data={"error": "Invalid Request of days"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        loan.due_date = loan.due_date + timedelta(days=additional_days_needed)

        return Response({"msg": "Loan Date Extended.."})
